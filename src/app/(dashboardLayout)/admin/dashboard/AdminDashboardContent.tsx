...
import { Users, Stethoscope, Calendar, DollarSign, Clock, CheckCircle, XCircle } from "lucide-react";
    ...
    <div className="space-y-8">
      ...
      <StatsGrid columns={4}>
        ...
        <StatCard
          ...
          value={adminMeta?.totalAppointments?.toLocaleString() ?? "0"}
          icon={Calendar}
          variant="warning"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <ProgressStat
              label="Scheduled"
              current={adminMeta?.appointmentByStatus?.scheduled ?? 0}
              total={adminMeta?.totalAppointments ?? 1}
              variant="default"
            />
            <ProgressStat
              label="In Progress"
              current={adminMeta?.appointmentByStatus?.inProgress ?? 0}
              total={adminMeta?.totalAppointments ?? 1}
              variant="warning"
            />
            <ProgressStat
              label="Completed"
              current={adminMeta?.appointmentByStatus?.completed ?? 0}
              total={adminMeta?.totalAppointments ?? 1}
              variant="success"
            />
            <ProgressStat
              label="Canceled"
              current={adminMeta?.appointmentByStatus?.canceled ?? 0}
              total={adminMeta?.totalAppointments ?? 1}
              variant="danger"
            />
          </CardContent>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Payment status summary</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-emerald-800 dark:text-emerald-400">Paid</span>
                </div>
              ...
              <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-100 dark:border-amber-900/50">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-amber-800 dark:text-amber-400">Unpaid</span>
                </div>
            ...
            <ProgressStat
              ...
              total={(adminMeta?.totalPaidAppointments ?? 0) + (adminMeta?.totalUnpaidAppointments ?? 0) || 1}
              variant="success"
            />
          </CardContent>
        </Card>
