...
import { User, Stethoscope, Calendar, Clock, CreditCard, FileText } from "lucide-react";
...
export function ViewAppointmentModal({ open, onOpenChange, appointment }: ViewAppointmentModalProps) {
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 rounded-lg space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{appointment.patient?.email || "N/A"}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">Contact</Label>
                <p className="font-medium">{appointment.patient?.contactNumber || "N/A"}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">Address</Label>
                <p className="font-medium">{appointment.patient?.address || "N/A"}</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 bg-green-50 rounded-lg space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium">{appointment.doctor?.designation || "N/A"}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">Email</Label>
              ...
              <div>
                ...
                <p className="font-medium">${appointment.doctor?.appointmentFee || "N/A"}</p>
              </div>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-medium">
                    {formatDateTime(appointment.schedule.startDateTime).date}
                  </p>
                </div>
                <div>
                  <Label className="text-muted-foreground">Time</Label>
                  <p className="font-medium">
                    {formatDateTime(appointment.schedule.startDateTime).time} - {formatDateTime(appointment.schedule.endDateTime).time}
                  </p>
                </div>
              </div>
            </div>
          ...
          {appointment.videoCallingId && (
            <div className="p-4 bg-orange-50 rounded-lg">
              <Label className="text-muted-foreground">Video Call ID</Label>
              <p className="font-medium text-sm break-all">{appointment.videoCallingId}</p>
            </div>
          )}
              ...
              <div className="space-y-2 text-sm">
                {appointment.prescription.instructions && (
                  <div>
                    ...
                    <p className="font-medium">{appointment.prescription.instructions}</p>
                  </div>
                )}
