...
export function EditSpecialtyModal({ open, onOpenChange, specialty }: EditSpecialtyModalProps) {
  const router = useRouter();
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [title, setTitle] = useState(specialty.title);
  const [iconFile, setIconFile] = useState<File | null>(null);
  const [iconPreview, setIconPreview] = useState<string | null>(specialty.icon || null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        toast.error(response.message || "Failed to update specialty");
      }
    } catch (error: any) {
