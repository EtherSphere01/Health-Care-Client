...
import { Eye, Video, FileText, CheckCircle, Calendar, Clock, User } from "lucide-react";
...
export function DoctorAppointmentsContent({
  ...
  const [viewingPatient, setViewingPatient] = useState<IAppointment | null>(null);
  const [prescriptionAppointment, setPrescriptionAppointment] = useState<IAppointment | null>(null);
  const [statusUpdateAppointment, setStatusUpdateAppointment] = useState<IAppointment | null>(null);
  ...
  const handleStatusUpdate = async () => {
    ...
    setIsUpdating(true);
    try {
      const response = await updateAppointmentStatus(statusUpdateAppointment.id, { status: newStatus });
      if (response.success) {
    ...
    },
    {
      ...
      cell: (appointment: IAppointment) => (
        <div className="flex items-center gap-2">
          ...
          {appointment.videoCallingId && appointment.status === "INPROGRESS" && (
            <Button
              variant="ghost"
              size="icon"
              onClick={() => window.open(appointment.videoCallingId, "_blank")}
          ...
          {appointment.status !== "COMPLETED" && appointment.status !== "CANCELED" && (
            <Button
              ...
              onClick={() => {
                ...
                setNewStatus(
                  appointment.status === "SCHEDULED" ? "INPROGRESS" : "COMPLETED"
                );
              }}
              title={appointment.status === "SCHEDULED" ? "Start Appointment" : "Complete Appointment"}
            >
    ...
    <div className="space-y-6">
      ...
      {prescriptionAppointment && (
        <CreatePrescriptionModal
          ...
          onOpenChange={(open) => !open && setPrescriptionAppointment(null)}
      ...
      <ConfirmDialog
        ...
        title={newStatus === "INPROGRESS" ? "Start Appointment" : "Complete Appointment"}
        description={
