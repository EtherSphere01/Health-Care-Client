...
import { getMyDoctorAppointments } from "@/services/appointment";
...
export default async function DoctorAppointmentsPage({ searchParams }: DoctorAppointmentsPageProps) {
  const params = await searchParams;
  
  return (
    <Suspense fallback={<Spinner size="lg" text="Loading appointments..." />}>
      <AppointmentsFetcher searchParams={params} />
    </Suspense>
  );
}

async function AppointmentsFetcher({ searchParams }: { searchParams: { page?: string; limit?: string; status?: string; paymentStatus?: string } }) {
