...
import { createDoctorSchedule, deleteDoctorSchedule } from "@/services/doctor-schedule";
...
export function MySchedulesContent({
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await deleteDoctorSchedule(scheduleToDelete.doctorId, scheduleToDelete.scheduleId);
  ...
  const columns = [
    {
      ...
      cell: (doctorSchedule: IDoctorSchedule) => (
        <div className="flex items-center gap-2">
          ...
          <span>{formatDate(doctorSchedule.schedule?.startDateTime || "")}</span>
        </div>
      ),
    },
    {
      ...
      cell: (doctorSchedule: IDoctorSchedule) => (
        <div className="flex items-center gap-2">
          ...
          <span>
            {formatTime(doctorSchedule.schedule?.startDateTime || "")} - {formatTime(doctorSchedule.schedule?.endDateTime || "")}
          </span>
        </div>
      ),
    },
    {
      ...
      header: "Status",
      cell: (doctorSchedule: IDoctorSchedule) => (
        <Badge variant={doctorSchedule.isBooked ? "destructive" : "success"}>
    ...
    },
    {
      ...
      cell: (doctorSchedule: IDoctorSchedule) => (
        <Button
          ...
          title={doctorSchedule.isBooked ? "Cannot remove booked schedule" : "Remove schedule"}
        >
