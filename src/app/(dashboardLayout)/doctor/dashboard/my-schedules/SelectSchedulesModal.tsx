...
export function SelectSchedulesModal({
  ...
  const groupedSchedules = unassignedSchedules.reduce((groups, schedule) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createDoctorSchedule({ scheduleIds: selectedScheduleIds });
      
      if (response.success) {
        toast.success(`${selectedScheduleIds.length} schedule(s) added successfully`);
        onOpenChange(false);
        setSelectedScheduleIds([]);
        router.refresh();
      } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>
            Choose the time slots you want to be available for appointments
          </DialogDescription>
        </DialogHeader>

        {unassignedSchedules.length === 0 ? (
          <div className="py-8 text-center text-muted-foreground">
            No available schedules to add. All schedules are already assigned.
          </div>
        ) : (
          <>
            {/* Select All */}
            <div className="flex items-center justify-between py-2 border-b">
              <span className="text-sm text-muted-foreground">
                {selectedScheduleIds.length} of {unassignedSchedules.length} selected
              </span>
              <Button variant="ghost" size="sm" onClick={handleSelectAll}>
                {selectedScheduleIds.length === unassignedSchedules.length
                  ? "Deselect All"
                  : "Select All"}
              </Button>
            </div>
