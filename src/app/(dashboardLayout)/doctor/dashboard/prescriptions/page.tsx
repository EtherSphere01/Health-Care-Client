...
import { getMyDoctorPrescriptions } from "@/services/prescription";
...
export default async function DoctorPrescriptionsPage({ searchParams }: DoctorPrescriptionsPageProps) {
  ...
  return (
    <Suspense fallback={<Spinner size="lg" text="Loading prescriptions..." />}>
      <PrescriptionsFetcher searchParams={params} />
    </Suspense>
  );
}

async function PrescriptionsFetcher({ searchParams }: { searchParams: { page?: string; limit?: string } }) {
  const page = parseInt(searchParams.page || "1", 10);
  const limit = parseInt(searchParams.limit || "10", 10);

  try {
    const response = await getMyDoctorPrescriptions({ page, limit });
