...
export async function getAllDoctorSchedules(
  ...
  return get<IDoctorSchedule[]>("/doctor-schedule", params as Record<string, unknown>, {
...
export async function getMyDoctorSchedule(
  ...
  return get<IDoctorSchedule[]>("/doctor-schedule/my-schedule", params as Record<string, unknown>, {
 ...
 */
export async function deleteDoctorSchedule(scheduleId: string): Promise<IApiResponse<IDoctorSchedule>> {
  const response = await del<IDoctorSchedule>(`/doctor-schedule/${scheduleId}`);
  revalidateTag(DOCTOR_SCHEDULES_TAG);
  revalidateTag("my-schedule");
  return response;
}
