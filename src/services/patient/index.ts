 ...
 * Get patient by ID
 */
export async function getPatientById(id: string): Promise<IApiResponse<IPatient>> {
  return get<IPatient>(`/patient/${id}`, undefined, { tags: [PATIENTS_TAG, `patient-${id}`] });
}
 ...
 */
export async function deletePatient(id: string): Promise<IApiResponse<IPatient>> {
 ...
 */
export async function softDeletePatient(id: string): Promise<IApiResponse<IPatient>> {
