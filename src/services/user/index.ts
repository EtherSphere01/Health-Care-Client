...
import { get, post, patch, del, uploadFormData, buildQueryString } from "@/lib/api";
import {
  IUser,
  IUserQueryParams,
  ICreateAdminRequest,
...
export async function createAdmin(
  ...
  const response = await uploadFormData<IUser>("/user/create-admin", formData);
...
export async function createDoctor(
  ...
  const response = await uploadFormData<IUser>("/user/create-doctor", formData);
  revalidateTag(USERS_TAG);
...
export async function createPatient(
  ...
  const response = await uploadFormData<IUser>("/user/create-patient", formData);
  revalidateTag(USERS_TAG);
...
export async function updateMyProfile(
  ...
  const response = await uploadFormData<IUser>("/user/update-my-profile", formData, "PATCH");
  revalidateTag("user");
